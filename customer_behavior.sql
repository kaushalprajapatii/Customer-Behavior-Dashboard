...
select subscription_status,round(avg(purchase_amount),2) as average_purchase ,  sum(purchase_amount) as total_revenue , 
...
-- with item_count as (
-- (select item_purchased,count(item_purchased) from customer
-- group by item_purchased 
-- order by count(item_purchased) desc 
-- limit 3 as items) 
-- )

-- select category ,item_count from item_count
-- group by category 

select category , item_purchased , count(customer_id) from customer
